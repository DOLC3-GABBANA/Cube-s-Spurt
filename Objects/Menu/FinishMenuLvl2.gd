...
func _ready():
	if data_handling.load_data("attempt.dat"):
		$Panel/AllAttempt.text = "YOUR ALL ATTEMPTS: " + String(data_handling.load_data("attempt.dat"))
	else:
		$Panel/AllAttempt.text = "YOUR ALL ATTEMPTS: " + String(globals.level_attempts)
	if data_handling.load_data("time.dat"):
		$Panel/AllTime.text = "YOUR ALL TIME: " + String(int(data_handling.load_data("time.dat")))
	else:
